...
MessageQueue::MessageQueue(const std::string& name, std::function<void(std::string)> callback) {
    _name = name.c_str();
    ...
    while(_running) {
        ...
        if (ms_data.msg_qnum > 0) {
            ...
            for (int i = 0; i < ms_data.msg_qnum; i++) {
                ...
                if (bytesRead == -1) {
                    Utils::LogFmt("MessageQueue::Recieve - Error recieving -  %s", strerror(errno));
                }
            }
