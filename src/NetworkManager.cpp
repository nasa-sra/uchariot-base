 ...
 *
 * This function sets up a TCP socket, binds it to the specified port, and
 * starts listening for incoming connections. It also creates a new thread to
 * handle incoming packets and invokes the provided packet callback function for
 * each received packet.
 *
 * @param port The port number to listen on.
 * @param packetCallback A callback function to be invoked for each received
 * packet.
 *
 * @return true if the network manager was successfully started, false
 * otherwise.
 *
 * @note If the socket creation, binding, or listening fails, an error message
 * is logged, and the function returns false.
 * @note The network manager runs in a separate thread, and the main thread
 * continues execution.
 * @note The network manager is stopped when the CloseConnections() function is
 * called.
 *
 * @see PacketCallback
...
bool NetworkManager::Start(int port, PacketCallback packetCallback) {
    ...
    int option = 1;
    setsockopt(_net_socket, SOL_SOCKET, SO_REUSEADDR, (char*)&option,
               sizeof(option));
 ...
 *
 * This function receives data from a client identified by the given file
 * descriptor (fd), processes the received data, and performs necessary actions
 ...
 * @return void
 *
 * @note If the number of bytes received (nbytes) is less than or equal to 0,
 * the function logs a disconnection message if nbytes is 0, or an error message
 * if nbytes is less than 0. It then removes the client from the list of
 * connected clients, updates the client count, closes the client's socket,
 * clears the client's file descriptor from the set, and adjusts the maximum
 * file descriptor if necessary.
 *
 * If the received data is from the commanding client, the function processes
 * the data by searching for semicolons to delimit individual packets and
 * invoking the handlePacket function for each valid packet.
 *
 * @see handlePacket
 */
 ...
 *
 * This function processes incoming packets from clients by parsing the raw data
 * and invoking the provided callback function with the extracted command and
 * data.
 *
 ...
 * @return void
 *
 * @note If the buffer contains no data or does not start with a '[' character,
 * the function returns immediately.
 * @note The function extracts the command and data from the raw input, parses
 * the data into a rapidjson::Document, and invokes the provided callback
 * function with the command and document.
 */
 ...
 *
 * This function iterates through all connected clients and sends the provided
 * data to each one. It utilizes the Send() function to send the data to each
 * client.
 *
 ...
 * @note If an error occurs during the send operation for any client, it logs an
 ...
 *
 * This function sends the data contained in the buffer to the client identified
 * by the given file descriptor. It uses a while loop to ensure all data is
 * sent, even if it spans multiple send calls.
 *
 ...
 * @return void
 *
 * @note If an error occurs during the send operation, it logs an error message
