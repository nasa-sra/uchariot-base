 ...
 * It uses the provided allocator to create the rapidjson::Value object.
 *
 * @param allocator The allocator to be used for creating the rapidjson::Value object.
 *
 * @return A rapidjson::Value object representing the stored value.
 *
 * @note The returned rapidjson::Value object will be of type rapidjson::kTrueType, rapidjson::kFalseType,
 * rapidjson::kNumberType, or rapidjson::kStringType, depending on the type of the stored value.
 */
...
void StateReporter::genericUpdateKey(std::string key, StateReporter::ValueEntry val) {
    ...
    if (it == _state.end()) {
        ...
        it->second->value = val;
        if (it->second->json != nullptr) val.setJsonValue(it->second->json, _doc.GetAllocator());
    }
}

/**
 * @brief Adds a new key-value pair to the state tree and associates it with a TreeNode.
 *
 * This function takes a key and a ValueEntry as input, climbs the state tree to find the appropriate
 * branch, creates a new TreeNode with the value name extracted from the key, and adds it to the branch.
 * The key-value pair is also stored in the _state map for quick access.
 *
...
/**
 * @brief Climbs the state tree to find the appropriate branch based on the given key.
 *
 * This function traverses the state tree starting from the given current node, using the key to
 * determine the next branch to visit. If the key contains multiple branches, it will recursively
 * call itself to climb deeper into the tree. If the key does not exist in the tree, a new branch
 * will be created and added to the current node.
 *
 * @param current The current node in the state tree to start climbing from.
 * @param key The key representing the path to the desired branch in the state tree.
 *
 * @return A pointer to the final branch node in the state tree, based on the given key.
 */
StateReporter::TreeNode* StateReporter::climbTree(TreeNode* current, std::string key) {
    int delim = key.find("/");
    if (delim == key.npos) { return current; }
    std::string nextBranch = key.substr(0, delim);
    auto it = std::find_if(current->branches.begin(), current->branches.end(),
 ...
 *
 * This function deletes a tree of TreeNode objects starting from the given node.
 * If the node is a fruit node (i.e., it has no branches), it is simply deleted.
 * If the node is a branch node, the function recursively calls itself to delete
 * all its child branches before deleting the node itself.
 *
...
void StateReporter::deleteTree(TreeNode* node) {
    ...
    for (TreeNode* branch : node->branches) { deleteTree(branch); }
    delete node;
}
 ...
 *
 * This function traverses a tree of TreeNode objects starting from the given current node,
 * and constructs a JSON document using the rapidjson library. If the current node is a fruit node
 * (i.e., it has no branches), it adds a member to the JSON document with the node's name and value.
 * If the current node is a branch node, it creates a new JSON object as a member of the JSON document,
 * and recursively calls itself to build the JSON document for each child branch.
 *
 * @param doc The rapidjson::Value object representing the JSON document to be built.
 * @param current The TreeNode object representing the current node in the tree.
 * @param allocator The rapidjson::Document::AllocatorType object used to allocate memory for the JSON document.
 *
 * @return void
 */
void StateReporter::buildDoc(rapidjson::Value& doc, TreeNode* current, rapidjson::Document::AllocatorType& allocator) {
    ...
    for (TreeNode* treeBranch : current->branches) { buildDoc(docBranch, treeBranch, allocator); }
 ...
 *
 * This function traverses a JSON document represented by a rapidjson::Value object,
 * starting from the given current node. It uses the provided key to determine the next
 * member to visit. If the key contains multiple members, it will recursively call itself
 * to climb deeper into the JSON document. If the key does not exist in the JSON document,
 * it logs an error message using the Utils::LogFmt function.
 *
 * @param doc The rapidjson::Value object representing the JSON document to be climbed.
 * @param key The key representing the path to the desired member in the JSON document.
 *
 * @return A rapidjson::Value::MemberIterator pointing to the desired member in the JSON document.
 * If the member is not found, it returns doc.MemberEnd().
 */
rapidjson::Value::MemberIterator StateReporter::climbDoc(rapidjson::Value& doc, std::string key) {
    int delim = key.find("/");
    if (delim == key.npos) { return doc.FindMember(key.c_str()); }
    std::string nextBranch = key.substr(0, delim);
 ...
 *
 * This function continuously sends the current state of the robot to a network destination
 * at a specified rate. It builds a JSON document from the state tree, sets the JSON document
 * value pointers in the state tree, and then sends the JSON document over the network.
 *
 * @return void
 *
 * @note This function runs in an infinite loop until the telemetry is disabled.
 * It checks the _stateRefreshed flag to determine whether the state has been updated since the last send.
 * If the state has not been updated, it sleeps for 1 millisecond before checking again.
 * It also uses a rate limiting mechanism to ensure that the sending rate does not exceed the specified rate.
 * If the sending rate is exceeded, it logs an error message.
 */
void StateReporter::sendState() {
    ...
    while (_telemetry) {
        ...
        if (dt > 1.0 / rate) { Utils::LogFmt("StateReporter sendState overran by %f s", dt); }
...
bool StateReporter::initLogFile() {
    ...
    for (auto i = _state.begin(); i != _state.end(); i++) { header += i->first + ", "; }
...
void StateReporter::logState() {
    ...
    std::string row = "";
    double time = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - _startTime)
                          .count() /
                  1000.0;
    row += std::to_string(time) + ", ";

    for (auto i = _state.begin(); i != _state.end(); i++) { row += i->second->value.toString() + ", "; }
