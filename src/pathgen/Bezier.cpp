...
void Bezier::_on_way_point_added() {
    ...
    if (new_control_point_index == 3) {
        ...
        if (new_control_point_index % 2 == 0) { return; }
...
/**
 * @brief This function calculates a point on a Bezier curve defined by four control points.
 *
 * @param t The parameter t in the range [0, 1] that determines the position on the curve.
 ...
 *
 * @return An `Eigen::Vector3d` representing the point on the Bezier curve at parameter t.
 *
 * The function uses the de Casteljau algorithm to calculate the point on the curve.
 * The algorithm is based on the recursive definition of Bezier curves.
 * The function calculates the point by summing the weighted control points, where the weights
 * are determined by the parameter t.
 */
Eigen::Vector3d Bezier::interpolate(double t, const Eigen::Vector3d& P0, const Eigen::Vector3d& P1,
