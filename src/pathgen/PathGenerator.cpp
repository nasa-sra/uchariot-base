 ...
 * A higher value results in a more detailed path, while a lower value results
 * in a less detailed path. The default value is 50.
 *
...
std::vector<Utils::GeoPoint> PathGenerator::GetRawPoints() {
    ...
    for (int i = 0; i < _pathPointsRaw.size(); i++) {
        bob.push_back(
            Utils::GeoPoint(_pathPointsRaw[i].x, _pathPointsRaw[i].y));
    }
...
/**
 * @brief Generates a path from a KML file, applies a specified speed and
 * radius, and saves the result as an XML file.
 *
 * This function reads a KML file, processes the path data, applies a specified
 * speed and radius, and generates a new path. The generated path is then saved
 * as an XML file, along with additional attributes such as speed, tolerance,
 * end tolerance, velocity kp, and heading kp.
 *
 ...
 */
int PathGenerator::GeneratePath(std::string filename, double speed_ms,
                                double radius_m) {
    std::vector<GenPoint> n_points;

    XMLDocument doc;
    int res = doc.LoadFile(
    ...
    for (int i = 0; i < points.size(); ++i) {
        ...
        Eigen::Vector3d prevVector =
            points[i] - (points[i] - points[i - 1]).normalized() * radius_m;
        Eigen::Vector3d postVector =
            points[i] + (points[i + 1] - points[i]).normalized() * radius_m;
    ...
    for (int i = 0; i < n_points.size() - 1; ++i) {
        if (n_points[i + 1].control || n_points[i].control ||
            i == (n_points.size() - 1)) {
            finPoints.push_back(n_points[i].toVector());
            continue;
        }

        Eigen::Vector3d halfway =
            n_points[i].toVector() +
            (n_points[i + 1].toVector() - n_points[i].toVector()) * 0.5;
        finPoints.push_back(n_points[i].toVector());
    ...
    for (int i = 0; i < curve->node_count(); i++) {
        ...
        if (i == (curve->node_count() - 1)) {
            tempPoint = Point(finPoints[finPoints.size() - 1],
                              curve->total_length(), speed_lat);
            pathFinalString.append(tempPoint.ToPointString());
        } else {
            tempPoint = Point(curve->node(i),
                              curve->length_from_starting_point(i), speed_lat);
            pathFinalString.append(tempPoint.ToPointString() + " ");
        }
